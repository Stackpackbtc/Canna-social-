...
type PendingVote = { name: string; type: 'GAS' | 'PASS' }
type VoteSuccess = { name: string; type: 'GAS' | 'PASS' }
...
export default function LiveVotes() {
  ...
  const filteredPool = useMemo(() => {
    if (filter === 'my') return pool.filter(([name]) => !!strainVotes[name])
    if (filter === 'trending') return pool.slice(0, 12)
    return pool
  }, [filter, pool, strainVotes])
  ...
  function showVoteSuccess(name: string, type: 'GAS' | 'PASS') {
    setLastAction(`Your ${type} vote on ${name} is counted`)
    setVoteSuccess({ name, type })
