...
const descriptions: Record<string,string> = {
  ...
  'Black Cherry Gelato':"Gibby’s Craft Cannabis Black Cherry Gelato. Gibby’s current catalog lists this cultivar at 24% THC with Black Cherry Pie × Acai Gelato lineage. Canna Social is opening this listing for public community voting.",
...
export default function StrainLibrary(){
  ...
  const getImage=(strain:Strain,index:number)=>strain[0]==='Black Cherry Gelato'?'/black-cherry-gelato-gibbys.svg':strain[6]||strainImages[(index+4)%strainImages.length]
  ...
  return <section className="strain-library" id="strain-library">
    ...
    <div className="strain-main">
      ...
      <div className="strain-library-head"><div><p className="eyebrow">✦ CANNA SOCIAL · DISCOVERY</p><h2>STRAIN <span>LIBRARY</span></h2><p className="library-subtitle">Explore strains, collections and profiles. See what the community is voting for.</p></div><div className="library-count"><strong>{strains.length}+</strong><span>STRAINS<br/>& GROWING</span></div></div>

      <div className="new-strains-banner"><div><span>✦</span><div><b>NEWLY ADDED</b><strong>{newCount}+ strains just landed</strong><small>Fresh additions to Canna Social · Now open for public voting</small></div></div><button onClick={()=>{setType('All');setSort('Top Rated');setQuery('')}}>Explore new strains ↓</button></div>
      ...
      <div className="library-grid">{filtered.map((strain,index)=>{const isCollection=strain[1]==='Collection';const isNew=strain[3]==='NEW'&&!isCollection;const image=getImage(strain,index);const brand=strainBrands[strain[0]];return <article className={`library-card ${isCollection?'collection-card':''} ${isNew?'new-strain-card':''}`} key={strain[0]}>
        <button className="strain-card-open" onClick={()=>setSelected(strain)} aria-label={`Preview ${strain[0]}`}><div className="strain-photo"><img src={image} alt={`${strain[0]} cannabis profile`} loading="lazy"/><div className="card-brand"><span>✦</span> {brand||'CANNA SOCIAL'} <em>{brand?'PARTNER':'LIBRARY'}</em></div><div className="rank-badge">{isCollection||isNew?'NEW':`#${index+1}`}</div><div className="card-gradient"/><div className="photo-overlay"><b>{strain[0]}</b><small>{brand||strain[1]}{strain[2]!=='—'?` · THC ${strain[2]}`:' · Profile'}</small><span>{strain[5]}</span></div></div></button>
        <div className="library-body"><div className="score-row"><div className="score"><strong>{isCollection?'★':isNew?'NEW':strain[3]}</strong><span><b>{isCollection||isNew?'PUBLIC VOTE':'COMMUNITY'}</b><small>{isCollection||isNew?'Be one of the first votes':`${strain[4]} votes`}</small></span></div><button className="library-vote" onClick={()=>vote(strain[0])}>{voted===strain[0]?'✓ VOTED':'VOTE'}</button></div>{!isCollection&&!isNew&&<div className="mini-bar"><span style={{width:`${Math.min(100,Number(strain[3])*10)}%`}}/></div>}<div className="card-links"><button onClick={()=>setSelected(strain)}>View {isCollection?'collection':'strain'}</button><button onClick={()=>setSelected(strain)}>{isCollection?'Preview →':'Info →'}</button></div></div>
      </article>})}</div>
    ...
    {selected&&(()=>{const selectedIndex=strains.findIndex((item)=>item[0]===selected[0]);const selectedImage=getImage(selected,selectedIndex);const isCollection=selected[1]==='Collection';const isNew=selected[3]==='NEW'&&!isCollection;const brand=strainBrands[selected[0]];return <div className="strain-preview-backdrop" role="dialog" aria-modal="true" aria-label={`${selected[0]} preview`} onClick={()=>setSelected(null)}><div className="strain-preview" onClick={(event)=>event.stopPropagation()}><button className="strain-preview-close" onClick={()=>setSelected(null)} aria-label="Close preview">×</button><div className="strain-preview-image"><img src={selectedImage} alt={selected[0]}/></div><div className="strain-preview-content"><p className="eyebrow">✦ CANNA SOCIAL · {isCollection?'COLLECTION':'STRAIN PROFILE'}</p><h3>{selected[0]}</h3>{brand&&<p className="tiny">PARTICIPATING BRAND · {brand}</p>}<div className="strain-preview-meta"><span>{selected[1]}</span>{selected[2]!=='—'&&<span>THC {selected[2]}</span>}<span>{isCollection||isNew?'NEW · PUBLIC VOTE':`${selected[4]} VOTES`}</span></div><p className="strain-preview-description">{descriptions[selected[0]]||`A Canna Social library profile featuring ${selected[5]}. This listing is newly added and open for community voting.`}</p><div className="strain-preview-flavors"><b>PROFILE</b><span>{selected[5]}</span></div>{!isCollection&&!isNew&&<div className="strain-preview-score"><strong>{selected[3]}</strong><span>/10 community score</span></div>}<div className="strain-preview-actions"><button className="library-vote" onClick={()=>vote(selected[0])}>{voted===selected[0]?'✓ VOTED':'VOTE NOW'}</button><button className="preview-secondary" onClick={()=>setSelected(null)}>CLOSE</button></div></div></div></div>})()}
  </section>
}
